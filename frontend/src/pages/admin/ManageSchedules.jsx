...
const runAutoSchedule = async () => {
  ...
  const getProcessedSchedules = () => {
    ...
    // Helper to parse time string "HH:MM AM - HH:MM PM"
    const getStartEnd = (timeStr) => {
    ...
    schedules.forEach(schedule => {
      ...
      if (isExtended) {
        let mergedTime = LAB_TIMES[schedule.time_slot_id];

        // If not in predefined LAB_TIMES, try to construct it from consecutive slots
        if (!mergedTime && TIME_SLOTS[schedule.time_slot_id]) {
           const currentSlotTime = TIME_SLOTS[schedule.time_slot_id];
           const nextSlotTime = TIME_SLOTS[schedule.time_slot_id + 1];
           
           if (currentSlotTime && nextSlotTime) {
             const { start } = getStartEnd(currentSlotTime);
             const { end } = getStartEnd(nextSlotTime);
             mergedTime = `${start} - ${end}`;
           } else {
             mergedTime = currentSlotTime; // Fallback
           }
        }
        ...
        processed.push({
          ...schedule,
          isMerged: true,
          mergedTime: mergedTime || TIME_SLOTS[schedule.time_slot_id],
          mergedDuration: "3h 10m"
        });
      } else {
        // Standard duration (1h 30m) - treat like theory
        processed.push({
            ...schedule,
            isMerged: false,
            mergedTime: TIME_SLOTS[schedule.time_slot_id]
        });
      }
    });
