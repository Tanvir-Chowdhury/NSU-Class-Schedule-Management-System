...
def ensure_tba_teacher(db: Session) -> Teacher:
    # Check if TBA teacher exists
    tba_teacher = db.query(Teacher).filter(Teacher.initial == "TBA").first()
    if tba_teacher:
        return tba_teacher

    # Check if TBA user exists
    tba_user = db.query(User).filter(User.email == "tba@system.local").first()
    if not tba_user:
        tba_user = User(
            email="tba@system.local",
