...
def clear_calendar(creds: Credentials):
    """
    Deletes all events created by this application from the user's primary calendar.
    """
    try:
        service = build('calendar', 'v3', credentials=creds)
        page_token = None
        while True:
            # Search for events with our specific tag in the description
...
def sync_schedule(user: User, creds: Credentials, db: Session):
    # First, clear existing events to avoid duplicates and ensure clean state
    clear_calendar(creds)
